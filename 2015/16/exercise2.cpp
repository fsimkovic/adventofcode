...
bool match(map<string, int> &expected, map<string, int> &person) {
	for (auto &f: expected)
		if (person.count(f.first)) {
			if ((f.first == "cats") || (f.first == "trees")) {
				if (person[f.first] <= f.second)
					return false;
			} else if ((f.first == "pomeranians") || (f.first == "goldfish")) {
					if (person[f.first] >= f.second)
						return false;
			} else if ((person.count(f.first)) && (f.second != person[f.first]))
					return false;
			}
	return true;
}
